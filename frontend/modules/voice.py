...
def inject_voice_fab() -> None:
    """
    Injects mic + voice FABs and the auto-record loop into the parent page.

    Architecture:
    - FAB elements & event listeners: injected ONCE (guarded by #rm-mic-fab presence).
    - Helper functions (arStart, arStop, shouldRecord …): re-defined on EVERY render so
      their closures stay alive even after Streamlit replaces the component iframe.
    - setInterval / resize listener: cleared and re-registered on EVERY render for the
      same reason — old iframe closures die when the iframe is torn down.
    - All mutable state lives on WIN (window.parent) so it survives reruns.
    """
